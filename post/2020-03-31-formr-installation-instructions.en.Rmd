...
- At this point, I realized that the next step was incorrectly listed on the formr documentation, and neglected to mention an important step of installing the cache server. - Use `sudo apt install opencpu-cache` to install the caching options for `nginx`. 
- Use `sudo nano /etc/nginx/opencpu.d/ocpu.conf` to edit the configuration file. 
- Remove the first `location` section and replace with: (NOTE very different than formr instructions)
...
Login to your mysql server with a user that has the right priviledges. I will say this part was hard because Lightsail doesn't provide root passwords, so I had to login via: `sudo mysql -uroot`.
